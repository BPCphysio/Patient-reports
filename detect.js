...
window.DETECT = (function () {
  ...
  function run(text, region) {
    ...
    // in the physio's own words; nothing is inferred.
    {
      ...
      const MECH_TH = /(?:หลังจาก|(?<!ปวด|เจ็บ|ตึง|เมื่อย|ชา|กล้ามเนื้อ|กระดูกสัน)หลัง|จาก|เพราะ|ตอน|ขณะ)\s*([^.,;\n0-9]{0,24}?(?:ตกบันได|ตก|ล้ม|หกล้ม|พลิก|บิด|อุบัติเหตุ|รถชน|ยกของ|วิ่ง|เล่น(?:กีฬา|บอล|ฟุตบอล|แบด|บาส|เทนนิส|กอล์ฟ)?|ซ้อม|ออกกำลังกาย|ผ่าตัด|ฉีดยา)[ก-๙]{0,20})/;   // the cause ends at the first space: Thai dictation has no punctuation, and the old 30-character tail swallowed "pain score 6/10 ไม่มีโรคประจำ…"
